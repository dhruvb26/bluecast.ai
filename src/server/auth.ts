...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  try {
    ...
    //! Find a better type for refresh token
      ...
      try {
        ...
        if (account && account.access_token) {
          ...
          if (response.ok) {
            ...
            await db.update(users).set({ headline }).where(eq(users.id, id));
            result[0].headline = headline;
          } else {
            console.error(
              `LinkedIn API request failed: ${response.statusText}`
            );
          }
        }
      } catch (error) {
